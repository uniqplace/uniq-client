...
export interface ProductUploadFormHandle {
...
const ProductUploadForm = forwardRef<ProductUploadFormHandle, ProductUploadFormProps>(
  ({ product, onClose, onComplete }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      validate: async () => {
        ...
        // בדיקה תקפה לפי הסכמה
        const isValidForm = await trigger();
        const hasImages = imageUrls.length > 0;

        if (!isValidForm || !hasImages) {
          if (!hasImages) setImageError('You must upload at least one image');
          return false;
        }
        ...
        try {
          ...
          let savedProductId = '';
          if (product) {
            const updated = await updateProduct({ _id: product._id, ...productData }).unwrap();
            savedProductId = updated?._id || product._id;
          } else {
            const created = await addProduct(productData).unwrap();
            savedProductId = created?._id;
          }
          // שמור תמיד productId במפתח תואם ל-stepper
          if (savedProductId) {
            const key = getUserProductKey(userId);
            localStorage.setItem(key, savedProductId);
          }
          onComplete?.();
          return true;
        } catch (err) {
          setUploadError('שמירת המוצר נכשלה');
          return false;
        }
      },
    }));
