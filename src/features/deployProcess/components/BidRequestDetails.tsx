...
const BidRequestDetails = () => {
    ...
    const creatorName = typeof bidRequest.creatorId === 'object' && bidRequest.creatorId !== null ? (bidRequest.creatorId as any).name : 'Unknown Creator';
    const creatorAvatar = typeof bidRequest.creatorId === 'object' && bidRequest.creatorId !== null ? (bidRequest.creatorId as any).avatarUrl : '';
                ...
                <div className="mb-4">
                    <div className="grid grid-cols-2 gap-6 items-center">
                        ...
                        <div className='flex items-center justify-center'>
                            <div className={`text-sm font-medium px-3 py-1 rounded-lg border w-fit ${bidRequest.status === 'open' ? 'bg-green-100 text-green-700 border-green-500' : bidRequest.status === 'expired' ? 'bg-yellow-100 text-yellow-700 border-yellow-500' : bidRequest.status === 'closed' ? 'bg-red-100 text-red-700 border-red-500' : 'bg-gray-100 text-gray-700 border-gray-300'}`}>{bidRequest.status || "Not specified"}</div>
                        </div>
                        <div className="font-semibold text-gray-700 flex items-center gap-2">
                            <i className="pi pi-tags text-gray-500 text-lg"></i>
                            <span className="text-lg">Category:</span>
                        </div>
                        <div className="text-sm text-gray-800 font-medium">{typeof bidRequest.categoryId === 'object' && bidRequest.categoryId !== null ? (bidRequest.categoryId as any).name : bidRequest.categoryId || "Not specified"}</div>

                        <div className="font-semibold text-gray-700 flex items-center gap-2">
                            <i className="pi pi-box text-gray-500 text-lg"></i>
                            <span className="text-lg">Product:</span>
                        </div>
                        <div className="text-sm text-gray-800 font-medium">{typeof bidRequest.productId === 'object' && bidRequest.productId !== null ? (bidRequest.productId as any).title : bidRequest.productId || "Not specified"}</div>
                        ...
                        <div className="text-sm text-gray-800 font-medium">{new Date(bidRequest.createdAt).toLocaleString('en-US')}</div>

                        <div className="font-semibold text-gray-700 flex items-center gap-2">
                            <i className="pi pi-refresh text-gray-500 text-lg"></i>
                            <span className="text-lg">Updated At:</span>
                        </div>
                        <div className="text-sm text-gray-800 font-medium">{new Date(bidRequest.updatedAt).toLocaleString('en-US')}</div>
                    </div>
                </div>
