...
const ProfilePage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    // Save manufacturer profile if role is manufacturer
    if (formData.role === 'manufacturer') {
      try {
        let result;
        if (user.role !== 'manufacturer') {
          result = await createManufacturerProfileMutation({
            userId: user.id ?? '',
            name: formData.name,
            categories,
            location,
          ...
          result = await updateManufacturerProfileByUserIdMutation({
            userId: user.id ?? '',
            name: formData.name,
            categories,
            location,
        ...
        dispatch(updateManufacturerProfile(result));
      } catch (error) {
        toast.current?.show({ severity: 'error', summary: 'Save Failed', detail: 'Failed to save manufacturer fields.', life: 3000 });
        return;
      }
